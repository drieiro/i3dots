...
#
# author: andreasl
# https://github.com/langenhagen/explore-with-dmenu
...
define_standard_settings() {
    ...
    choices=(
        ...
        '.'
        '..'
        "$(ls --group-directories-first "$selected_path")"
        "$(cat "$history_file")"
    )
...
while : ; do
    ...
    if [ "$dmenu_result" == '<abrir un terminal aquí>' ]; then
    ...
    if [ -f "$selected_path" ] || [ "$dmenu_result" = '.' ]; then
        ...
        choices=( '<abrir un terminal aquí>' '.' '..' "$(ls --group-directories-first "$selected_path")")
    else
